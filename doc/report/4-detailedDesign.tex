...
\begin{itemize}
	...
	\item \texttt{PrivateRoomSupport}: used to set a password to the room in order to prevent access to some clients.
...
\texttt{ClientRoom} is the client-side representation of the room. It extends the \texttt{Room} trait so it has a unique identifier \textit{roomId} and a set of properties (\ref{fig:property-class}) that can be queried by the user. 
...
This separation was made to provide the developer a consistent access to room functionalities. Indeed, with this structure is not possible for example to leave a room that was never joined or joining a room that is still joined.
\\
