...
\item \textbf{Join by Id}: a client joins a room by providing its id, and optionally a password(required if the room is private, any provided password will be ignored if the room is public).
...
\textit{Leave}
\\
A client can leave a room. Obviously, the operation fails if the client didn't previously join the room. 
