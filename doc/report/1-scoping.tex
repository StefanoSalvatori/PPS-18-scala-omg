...
\begin{itemize}
\item Idle timeout: The room kicks a client out (considering this as a leave event) if no message flew between such client and the room (both directions) within a certain period of time. The length of the period must be configurable and optionally disabled.
\item Heartbeat service: clients and server periodically exchange ping-pong messages: if a client misses a pong he is kicked out from the room (considering this as a leave event). The heartbeat rate must be configurable and optionally disabled.
\end{itemize}
When heartbeat and idle timeout are enabled at the same time, pong messages are considered as valid messages from the client so in this case the idle timeout could be triggered only if his rate is lower then the heartbeat one.
...
A client should be able, in primis, to display all existing rooms of a given type, both for public and private rooms, using two different ways, that is:
\begin{itemize}
...
\item \textbf{Error event}: the client is notified when errors occur in the communication with the room.
\end{itemize} 
...
\begin{figure}[h]
  \includegraphics[scale=0.4]{images/2-scoping/server-RBS.png}
   \centering  
   \caption{\textit{RBS for server side user requirements.}}
  \label{fig:server-RBS}
\end{figure}
\begin{figure}[h]
  \includegraphics[scale=0.4]{images/2-scoping/client-RBS.png}
   \centering  
...
9.4     & Define behavior on communication error\\
10      & Close a room \\
...
\subsection{Non functional Requirements} \label{non-fun-req}
