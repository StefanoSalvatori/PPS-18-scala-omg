...
import client.utils.{BasicActor, HttpService, SocketFailException, SocketService}
...
class MatchmakingActorImpl(private val roomType: RoomType,
  ...
  extends MatchmakingActor with Stash with SocketService {
  ...
  def onReceive: Receive = {
    case JoinMatchmaking =>
      makeSocketRequest(BinaryProtocolSerializer(), Routes.matchmakingSocketConnection(this.roomType), this.httpServerUri)
